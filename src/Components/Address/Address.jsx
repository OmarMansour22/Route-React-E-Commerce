...
export default function Address() {
    ...
    const validationSchema = Yup.object({
        name: Yup.string().required("Name is required.").min(2, "Name must be at least 2 characters.").max(20, "Name cannot exceed 20 characters.").required("Name is required."),
            ...
            <div className="bg-white shadow-card dark:shadow-darkCard dark:bg-black dark:border-neutral-700 rounded-xl p-6 max-w-lg mx-auto border border-gray-200 address-container" ref={addressContainerRef}>
                ...
                <div className="space-y-4">
                    {addresses.map(addr => (
                        <div key={addr._id} className={`flex justify-between items-center p-4 border dark:border-neutral-700 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-neutral-900 transition ${selectedAddress?._id === addr._id ? "border-blue-500" : "border-gray-300"}`} onClick={() => { setSelectedAddress(addr); setIsNewAddress(false); }}>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); removeAddress(addr._id); }} className="text-red-500 hover:text-red-700 cursor-pointer">Remove</button>
                        </div>
                    ))}
                </div>
                ...
                {isNewAddress && (
                    <form className="space-y-3 mt-4" onSubmit={formik.handleSubmit}>
                        ...
                        <input type="text" name="details" placeholder="Address Details" {...formik.getFieldProps("details")} className="p-3 w-full border dark:text-white dark:autofill:bg-neutral-900 dark:border-neutral-700 rounded-lg border-gray-500 outline-gray-700 text-gray-800" />
                        {formik.touched.details && formik.errors.details && <div className='text-red-500'>{formik.errors.details}</div>}

                        <input type="text" name="phone" placeholder="Phone Number" {...formik.getFieldProps("phone")} className="p-3 w-full border dark:text-white dark:autofill:bg-neutral-900 dark:border-neutral-700 rounded-lg border-gray-500 outline-gray-700 text-gray-800" />
                        {formik.touched.phone && formik.errors.phone && <div className='text-red-500'>{formik.errors.phone}</div>}

                        <input type="text" name="city" placeholder="City" {...formik.getFieldProps("city")} className="p-3 w-full border dark:text-white dark:autofill:bg-neutral-900 dark:border-neutral-700 rounded-lg border-gray-500 outline-gray-700 text-gray-800" />
                        {formik.touched.city && formik.errors.city && <div className='text-red-500'>{formik.errors.city}</div>}

                        <button type="submit" className="w-full py-3 bg-green-500 text-white font-semibold rounded-lg hover:bg-green-600 transition">Save Address</button>
                    </form>
                )}

                {selectedAddress && !isNewAddress && (
                    <button onClick={handleCheckout} className="w-full mt-6 py-3 cursor-pointer bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition" disabled={isLoading}>
                        Checkout {isLoading && <i className="fas fa-spinner fa-spin  flex items-center justify-center py-1.5 px-1"></i>}
                    </button>
                )}
            </div>
        </>
